...
纽约大学石溪分校的 Nguyen Phong Hoang 和多伦多大学的 Arian Akhavan Niaki 等人，建立了一个名为 GFWatch 的网络平台，对中国网络长城（俗称 GFW）的 DNS 审查系统进行了探测和实验，最后写出了一篇论文发表在历史悠久的 USENIX（高等计算系统协会）上。文章名为《How Great is the Great Firewall ?  Measuring China’s DNS Censorship》。
...
*GFW 审查不同域名所需时间的巨大差异表明，封锁名单很可能是由自动工具和人工共同完成的。*
...
通过分析每个伪造 IP 地址的注入频率，我们发现并不是所有的伪造IP都是平等地注入到被审查的回应中，也就是说，他们的注入模式并不是完全随机的。
...
## 围城：双向拦截
先前，研究人员认为 GFW 的双向 DNS 注入行为是中国以外的公共 DNS 解析器缓存被污染的原因。经过进一步的研究，研究人员发现针对权威名称服务器位于中国的许多域名的 DNS 中毒是另一个主要原因，即中毒的DNS记录已经玷污了世界各地的许多公共DNS解析器。

然后，我们展示了 GFWatch 发现的被审查的域名和伪造的IP的数据集如何有助于检测和净化公共DNS解析器缓存中的中毒资源记录。

### GFW 对中国域名的地理封锁
2020 年 8 月 8 日，GFWatch 检测到这样一个奇怪的封锁行为：GFW 对中国政府网站进行审查。

www.beian.gov.cn 由中国工业和信息化部管理，这个域名有两个权威的名称服务器，dns7.hichina.com 和 dns8.hichina.com，它们被托管在 16 个不同的中国境内的 IP 地址上。一旦在中国境外发出向针对该网站的 DNS 查询，GFW 便会对查询进行污染与注入。但位于中国境内的主机仍然可以正常访问这个网站。
...
写到这里的时候，我已经很累了，我已经写了四千多字了，感觉有点写不动了。

最后，研究人员们针对前文提到的内容，提出了一套反制现今 GFW 的 DNS 污染的方法。



10 结论
在这项工作中，我们开发了GFWatch，一个大规模的纵向测量平台，以提供一个不断更新的GFW基于DNS的封锁行为及其对全球互联网的影响。在9个月的时间里，GFWatch测试了534M个域名，发现了311K个被审查的域名。

我们发现，GFW的DNS审查制度对全球互联网，特别是域名生态系统产生了广泛的负面影响。GFWatch已经检测到超过77K的审查域名，其有毒的资源记录已经污染了许多流行的公共DNS解析器，包括谷歌和Cloudflare。基于从GFWatch收集的数据中获得的洞察力，我们随后提出了有效检测中毒响应和规避GFW的DNS审查的策略。

随着GFWatch的继续运作，我们的数据不仅会给技术观察带来新的启示，也会及时地让公众了解GFW封锁政策的变化，并协助其他检测和规避工作。
